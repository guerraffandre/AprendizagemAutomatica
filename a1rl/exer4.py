...
from Funcs import Stdev, PrintMatrix, StateTransition, RandomAction, IsParede, BuildMatrix, UpdateReward, PrintMatrixNoWalls, GetMatrixOnlyRewards, Test1000, GetBestStepAction, BuildInsideWalls
...
runs = 60 # alinea 2b = 40
steps = 1500 # alinea 2b = 300

matrix = [[0 for _ in range(cols)] for _ in range(rows)]

BuildMatrix(matrix,cols,rows,reward)
BuildInsideWalls(matrix, rows, cols)
...
for r in range(runs):
    ...
    while currentState != finalState:        
        ...
        nextState = StateTransition(currentState, RandomAction()) #Exericio 2a
        #nextState = GetBestStepAction(matrix, currentState)#Exericio 2b
